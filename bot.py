...
@dp.message()
async def snf(message: Message):
    fio = message.text                      #ФИО от пользователя
    snf = transliterate_fio(fio)   #Транслитерация ФИО
    await message.reply(snf)       #Отправляем ФИО пользователю

    # Логируем полученное ФИО и его транслитерацию
    logging.info(f'Пользователь {message.from_user.username} отправил ФИО: {fio}, транслитерация: {snf}')
